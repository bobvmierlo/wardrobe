...
Run it against the container's database (the -T is required: without it
``docker compose exec`` opens a TTY and refuses the piped script):

    docker compose exec -T kledingkast python - < scripts/check_db.py

or locally, pointing at your own database file:
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(f"Kan de database niet lezen ({DB}): {exc}")
